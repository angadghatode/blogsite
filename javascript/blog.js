...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize the GitHub Calendar widget
    if (typeof GitHubCalendar !== 'undefined') {
        // REPLACE 'angadghatode' with your exact GitHub username if it differs
        GitHubCalendar(".github-chart-container", "angadghatode", {
            responsive: true,
            tooltips: true
        });
    }
...
function editCurrentPost() { 
    if (currentPost && authenticated) openWrite(currentPost); 
}
...
document.addEventListener('keydown', e => {
    if (e.key === 'Escape') {
        ...
        if (document.getElementById('sidebar')?.classList.contains('mobile-open')) { toggleSidebar(); return; }
    }
    
    // CMD/CTRL + K to open new entry dialog
